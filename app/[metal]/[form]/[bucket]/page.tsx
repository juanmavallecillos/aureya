...
export async function generateMetadata({
  params,
}: { params: { metal: string; form: string; bucket: string } }): Promise<Metadata> {
  const metal = toMetalToken(params.metal) as "gold"|"silver";
  const form  = toFormToken(params.form) as "bar"|"coin";
  ...
  const description =
    `Compara precios y primas vs spot para ${h1Form.toLowerCase()} de ${h1Metal.toLowerCase()} ${bucket}. Tiendas verificadas y datos claros.`;
...
export default async function CategoryBucketPage({ params }: { params: { metal: string; form: string; bucket: string } }) {
  const metal = toMetalToken(params.metal) as "gold" | "silver";
  const form  = toFormToken(params.form) as "bar" | "coin";
  const bucket = normalizeBucket(params.bucket);
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 pb-16">
      ...
      <section className="pt-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            {/* Atajos contextuales */}
            {/* <nav aria-label="Atajos" className="mt-3">
      ...
      {/* TABLA prefiltrada (facets de metal/form/bucket ocultas) */}
